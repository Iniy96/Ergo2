...
//get for formdata 1
app.get('/project_details/formdata1', (req, res) => {
  connection.query('SELECT Sales_Order_No, Po_No, Po_Days, Po_Date, Quote_No, Sales_Order_Date, Service_Component, Document_Date, Rig_Name, Customer_Name, Rig_Location, Rig_Type, Estimated_Date_Of_Commencement, Estimated_Project_Completion_Month FROM project_details', (err, results) => {
      if (err) {
          console.log(err);
          res.send({ success: false, message: "Error fetching data" });
      } else {
...
app.get('/project_details/formdata2', (req, res) => {
  const { Supervisor1, Technician1 } = req.query;

  let query = 'SELECT Supervisor1, Technician1 FROM project_details';

  // Check if supervisor and technicians parameters are provided
  if (Supervisor1 && Technician1) {
      query += ` WHERE Supervisor1 = '${Supervisor1}' AND Technician1 = '${Technician1}'`;
  } else if (Supervisor1) {
      query += ` WHERE Supervisor1 = '${Supervisor1}'`;
  } else if (Technician1) {
      query += ` WHERE Technician1 = '${Technician1}'`;
  }
...
app.get('/project_details/formdata3', (req, res) => {
  connection.query('SELECT Tool1, Item_ID1, Make1, Model1, Serial_Number1, Calibration_Date1, Calibration_Due_Date1, Acceptance_Criteria1, Tool2, Item_ID2, Make2, Model2, Serial_Number2, Calibration_Date2, Calibration_Due_Date2, Acceptance_Criteria2 FROM project_details', (err, results) => {
      if (err) {
          console.log(err);
          res.send({ success: false, message: "Error fetching data" });
      } else {
